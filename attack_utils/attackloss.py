        ...
        if tloss > 0:
            ...
            if (yloss > 0):
                ...
                normalized_grad_yolo = grad_np_yolo / np.sqrt(
                    np.sum(grad_np_yolo * grad_np_yolo, axis=(2, 3), keepdims=True))  # l2
            else:
            ...
            if (rloss > 0):
                ...
                normalized_grad_rcnn = grad_np_rcnn / np.sqrt(
                    np.sum(grad_np_rcnn * grad_np_rcnn, axis=(2, 3), keepdims=True))  # l2
            else:
...
def ada_attack(yolo_model, rcnn_model, img, conf_thresh, max_iter, epsilon, mask):
    ...
    for iter_n in range(max_iter):
        ...
        ind_one = np.logical_and(NOISE >= 0, NOISE <= 1)
        ind_minus_one = np.logical_and(NOISE >= -1, NOISE < 0)
        NOISE = ind_one * 1 + ind_minus_one * (-1) + NOISE * (1 - ind_one - ind_minus_one)
        if iter_n == 0:
