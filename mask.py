...
for i in range(len(imgs_list)):
    ...
    Kflag = np.load('/home/yanhuanqian/ATRCNN/SEED/{}_Kflag.npy'.format(image_name)).tolist()
    mask = np.load('/home/yanhuanqian/ATRCNN/SEED/{}_mask.npy'.format(image_name))
    _, noise = L2_attack(darknet_model,rcnn_model, img, conf_thresh=0.4, max_iter=100, epsilon=200, mask=mask)
    mask = TopTen(noise,Kflag,0.6)
    np.save('Mask/{}.npy'.format(image_name),mask)
print('done...')